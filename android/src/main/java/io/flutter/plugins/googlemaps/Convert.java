...
class Convert {
  ...
  // Use 1/6th of the available memory for the cache
  private static final int MAX_MEMORY = (int) (Runtime.getRuntime().maxMemory() / 1024);
  private static final int CACHE_SIZE = MAX_MEMORY / 6;

  private static LruCache<String, BitmapDescriptor> bitmapCache = new LruCache<>(CACHE_SIZE);
...
public static BitmapDescriptor getBitmapFromAsset(
    ...
    final Double width = assetDetails.containsKey(widthKey) ? toDouble(assetDetails.get(widthKey)) : null;
    final Double height = assetDetails.containsKey(heightKey) ? toDouble(assetDetails.get(heightKey)) : null;

    int targetWidth = width != null ? toInt(width * density) : 0;
    int targetHeight = height != null ? toInt(height * density) : 0;

    // Create a unique cache key based on the asset details and target dimensions
    String cacheKey = assetKey + "_" + scalingMode + "_" + density + "_" + targetWidth + "_" + targetHeight;

    // Check if the bitmap is already in the cache
    BitmapDescriptor cachedBitmapDescriptor = bitmapCache.get(cacheKey);
    if (cachedBitmapDescriptor != null) {
        return cachedBitmapDescriptor;
    }
        ...
        case "auto":
            ...
            try {
                ...
                if (width != null || height != null) {
                    ...
                    bitmapDescriptor = bitmapDescriptorFactory.fromBitmap(toScaledBitmap(bitmap, scale));
                }

                // Add the bitmap descriptor to the cache
                bitmapCache.put(cacheKey, bitmapDescriptor);
    ...
    BitmapDescriptor bitmapDescriptor = bitmapDescriptorFactory.fromAsset(assetKey);
    bitmapCache.put(cacheKey, bitmapDescriptor);
