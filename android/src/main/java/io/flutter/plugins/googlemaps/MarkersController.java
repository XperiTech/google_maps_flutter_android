...
class MarkersController {
  ...
  void addMarkers(@NonNull List<Messages.PlatformMarker> markersToAdd) {
    ...
    //         final int index = i;
    //         handler.postDelayed(() -> {
    //             Messages.PlatformMarker markerToAdd = markersToAdd.get(index);
    //             addJsonMarker(markerToAdd.getJson());
    //         }, i * 10); 
    //     }
  ...
  private void fadeInMarker(final Marker marker) {
        marker.setAlpha(0f);
        final ValueAnimator fadeAnim = ValueAnimator.ofFloat(0, 1);
